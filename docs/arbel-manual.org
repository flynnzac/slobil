...
A line of ARBEL code is called a statement.  A statement consists of a sequence of data and syntax objects each separated by whitespace, ending with a period. 

In most cases, the first data element in a statement will be an /operation/. An operation is typically a built-in ARBEL command to perform some action.  They implement the basic functionality necessary to manipulate different types of data and to move data in to and around in registries.  They take a number of positional arguments.
...
There are no truly reserved keywords in ARBEL.  The standard operations are located at certain registers in the top-level registry by default, but they are data just like data you create yourself.  You can move them around in the registry, in effect changing the name of the operation.  This is like a bad idea (at least in the top-level registry), but ARBEL will let you do it.

Aside from Strings, some other basic data types in ARBEL which will be useful in the coming examples are: Integers, Reals, Registers, and Booleans.  Integers are numbers without a decimal point.  Reals are numbers with a decimal point.  Registers are symbols denoting locations in a registry. Booleans are either =True= or =False=.  There are a few other types but those will be introduced in their own sections.
...
Registers are the locations of data in a registry. Because all data is stored in a registry, they identify data and variables in ARBEL.  

Registers are themselves a type of data and can be stored in other registers.  For example,
#+BEGIN_SRC arbel
... set /x /y .
... set x 10 .
... print y .
...
In the first statement, =/x= is set to contain the register =/y=.  In the second statement, the =set= operation expects a register as its first argument, and the argument is provided by referencing the /value/ located at =/x=, which is =/y=.  So the =set= operation assigns the value =10= to =/y=.

Registers are used for operations where the location of data is what matters, not the data itself.  For example, data can be moved around from register-to-register using the =move= operation.  When data is moved in this way, it is not copied so it is an efficient way to reassign a variable name.  
...
Registers are the location of data in /registries/.  When you start up ARBEL, you are in the top-level registry, but you can, and likely will, create your own registries to structure your data.  Registries are also used to determine "scope".  Which registry you are inside, determines which values are assigned to which registers.  This section discusses the basics of using registries.

To create a registry, use the =registry= operation.  The =registry= operation sets the =/ans= register in the current registry to a new registry.  The =/ans= register is a special register.  Many operations set the =/ans= register in the current registry to a result.  This will be discussed in more detail later on in this section.

For an example of using the =registry= operation,
#+BEGIN_SRC arbel
...
The =get= operation sets the =/ans= register to the value located at a certain register in a registry.
#+BEGIN_SRC arbel
... get /z my-registry .
ans = hello
#+END_SRC
...
The colon notation can also be nested and registers can be referenced by looking at the value of other registers.  For example,
#+BEGIN_SRC arbel
... set /inside [ registry /y 12 . ] my-registry .
... print my-registry:/inside:/y .
12
... set /reg /z .
... print my-registry:reg .
hello
#+END_SRC

This example introduces the optional third argument of the =set= operation.  =set= can be used to either set registers in the current registry (the default) or set values in other registries by including the registry as the third argument.

It also introduces the square brackets which we have yet to see, but which, you might guess, denote a substatement.  We will discuss these in detail in the next section.

** Parenthesis

Parenthesis are an important element of ARBEL code.  There are three different kinds of parenthesis.  They all enclose statements, but they differ in /when/ those statements are executed.  Briefly, statements enclosed in square brackets (=[]=) are executed immediately before the outer statement is run and insert the value to which those statements set the =/ans= register into the original statement; statements enclosed in parenthesis (=()=) are saved as a data type called an /instruction/, are not executed immediately, but can later be executed by the user in other contexts; and statements enclosed in curly braces (={}=) are only executed if the outer statement actually tries to use them as an argument in which case they evaluate to what they set the =/ans= register to. We will cover each of them in this section

*** Square Brackets

When statements are enclosed in square brackets, the statements they contain are executed immediately, and the value they set the =/ans= register to becomes the value included at that position in the other statement. All substatements enclosed in square brackets must set the =/ans= register.  Otherwise, there is an error.

For an example using square brackets,
#+BEGIN_SRC arbel
... add 1 [ sub 5 2 . ] .
ans = 4
#+END_SRC

=add= is the operation to add numbers and =sub= is the operation to subtract numbers.  Both set the =/ans= register to the result. The square brackets tell ARBEL to first subtract =2= from =5= and store the result in the =/ans= register.  Then, ARBEL inserts the value in the =/ans= register where the substatement appears in the code so the statement becomes:
#+BEGIN_SRC arbel
add 1 3 .
#+END_SRC

It then evaluates the outer statement.

The important thing to remember about statements inside square brackets is that they are /always/ executed, even if the operation does not end up using that argument.  They are executed before the outer statement.  

An important corner case: statements in square brackets are always evaluated in the order they appear in the outer statement.  It is probably bad practice to depend on this feature or to use substatements that might affect the evaluation of other substatements in the same line of code.

Substatements in ARBEL are not necessarily single lines of code.  You can include very long substatements with complicated logic. 
...
The register =/add-one= is set to hold the quoted code/instruction. Instructions can then be "called" using the =call= operation.  For example:
...
Calling an instruction creates a new temporary registry in which the instruction's code is executed.  The arguments following the instruction in the =call= operation determine the initial state of this temporary registry.  The caller has complete freedom in determining the initial state of the registry.  In the above code, the author of the =add-one= instruction likely had in mind that the user would supply the argument =/t=.  But there is nothing stopping the user from replacing the =add= operation with another operation of their choosing like =sub= to subtract one instead of adding it.  The instruction is as generic as the caller of the instruction would like it to be.  The only part of the instruction that cannot be replaced by the caller is literal data (like =1=).

After the code finishes whatever the =/ans= register is set to in the temporary registry is the new value of the =/ans= register in the calling registry.  This is the sense in which the =/ans= register is special because this will not happen with the other data in the temporary registry.

To see the code used in an instruction, you can =print= the instruction.
#+BEGIN_SRC arbel
... print add-one .
(  add t 1 .  )
#+END_SRC
