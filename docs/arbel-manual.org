...
Registers in ARBEL are symbols and locations.  Registers are locations in a registry. Because all data is stored in a registry, they identify data and variables in ARBEL.  
...
An important corner case: statements in square brackets are always evaluated in the order they appear in the outer statement.  It is probably bad practice to depend on this feature or to use substatements that might affect state in other ways, but hey, if it solves your problem quickly, then you can write whatever you'd like in the substatement.
