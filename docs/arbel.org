...
*** Operations on "list" registers: /#1, /#2, ...
- =next REGISTER= --- if the =REGISTER= ends in a number, return the Register with that number incremented by 1.

- =previous REGISTER= --- if the =REGISTER= ends in a number, return the Register with that number minus 1.  If that would cause the number to be less than 1 set the =/ans= Register to the "first" register with that prefix so that =previous /#1 .= sets =/ans= to =/#1=.
