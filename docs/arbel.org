...
** Example 1: A Line-Oriented Text Editor
...
ans = hellohellohellohellohelloh
#+END_SRC
...
- =is-nothing VALUE= --- sets the =/ans= Register to =True= if the =Value= is a Nothing and to =False= otherwise.
...
- =substring STRING INTEGER1 INTEGER2= --- sets the =/ans= Register to the subset of STRING where the characters included are determined by INTEGER1 and INTEGER2.  They follow the same indexing rules as other String functions.

- =up INSTRUCTION= --- call INSTRUCTION in the Registry above the current Registry.
