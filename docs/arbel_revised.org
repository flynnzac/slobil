...
The key thing to understand about tasks is that their environment is entirely /separate/ from the current registry.  There is no way to directly access data in a task.  Data can only be accepted and retrieved by using the =accept= operation and 
