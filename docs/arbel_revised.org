...
Everything in ARBEL is data, including the scope in which code is executed.  "Data sets" called registries in ARBEL determine what variables are available to ARBEL code at any given time. Registries are flexible tables containing data located at /registers/.  These data locations are themselves data which enables many simple optimizations such as moving data between registers without copying. Registries function both as the scope of code and are the main data structure in ARBEL.
...
Initially, the code you enter at this prompt is executed in the /root/ registry.  Later, it may be executed in other registries.

An ARBEL statement with no substatements has a highly regular structure.  It consists of a number of data values each separated by whitespace ending with a period, itself separated from the last data element by whitespace.
#+BEGIN_SRC arbel
... data1 data2 data3 data4 .
#+END_SRC

The first element in an ARBEL statement is usually an /operation/.  Operations perform actions using the rest of the data elements in the statement. The first element can also be an /instruction/ which is the data type for ARBEL code.  Instructions perform actions using the data elements when they are used in this way, but they are more flexible than operations and can be used in other contexts.
...
The first statement uses the operation located at the =add= register.  The second uses the operation located at the =print= register.  The numbers and the the string ="Hello"= are data literals.
