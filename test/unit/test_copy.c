...
START_TEST(test_assign_slot)
{
  data* d = NULL;

  assign_slot(&d, "test", hash_str("test"));

  ck_assert_str_eq(((slot*) d->data)->name, "test");
  ck_assert_str_ne(((slot*) d->data)->name, "test1");
  ck_assert_uint_eq(((slot*) d->data)->key, hash_str("test"));
...
wob_copy_suite ()
{
  ...
  tcase_add_test(tc_core, test_assign_slot);
