...
* TODO Basic inefficiency to eliminate right now is in how "lookups" are performed, lots of room for improvement instead of scanning unordered linked list (maybe)  Let's try a hash table implementation
...
Where entry is basically current registry without the linked list part

get is then:

for (int i = 0; i < registry->num; i++)
{
	if (registry->key[i] == hash)
  {
    return registry->r[i]->data;
  }
}
