...
* TODO Make registries true hash table (i.e. number of bins grows with entries)

* TODO Consider name change to something hash based.  Maybe switch from "registry" terminology?  Although I kind of like that.

* TODO Some way to "capture" errors so they do not stop interpretation

* TODO There is something that creates a registry without using new_registry and it does not set size
